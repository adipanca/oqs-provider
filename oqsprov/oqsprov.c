...
int oqs_patch_oids(void) {
    ///// OQS_TEMPLATE_FRAGMENT_OID_PATCHING_START
    {
        ...
        if ((envval = getenv("OQS_OID_XWING768")))
            oqs_oid_alg_list[106] = envval;

        if ((envval = getenv("OQS_OID_P384_XWING768")))
            oqs_oid_alg_list[108] = envval;
        if ((envval = getenv("OQS_OID_X448_XWING768")))
            oqs_oid_alg_list[110] = envval;
        if ((envval = getenv("OQS_OID_X25519XWING768")))
            oqs_oid_alg_list[112] = envval;
        if ((envval = getenv("OQS_OID_SECP256R1XWING768")))
...
// clang-format off
    ...
    KEMBASEALG(xwing768, 192)
    KEMHYBALG(p384_xwing768, 192)
    KEMHYBALG(x448_xwing768, 192)
    KEMHYBALG(X25519XWING768, 128)
    KEMHYBALG(SecP256r1XWING768, 128)
#endif
...
#ifdef OQS_ENABLE_KEM_xwing_768
    KEMKMALG(xwing768, 192)

    KEMKMHYBALG(p384_xwing768, 192, ecp)
    KEMKMHYBALG(x448_xwing768, 192, ecx)
    KEMKMHYBALG(X25519XWING768, 128, ecx)
    KEMKMHYBALG(SecP256r1XWING768, 128, ecp)
#endif
